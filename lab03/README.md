...
按课程指南实现字符级 MiniGPT。作者：孙驰（2024311281，计算机10班）。实际AI协作工具：Codex。代码、原始结果及按新版模板填写的 [Word/PDF报告](report/) 均已完成。
...
- 八个1000步训练变体全部完成，保留基线1000步快照用于同更新次数比较；上下文变化时token数与计算量不同。
...
|`report/`|正式报告、主观质量评级和提交前独立复查记录|

局限：固定种子只说明此初始化；模型大小变化不能保证收益。嵌入宽度组按指南同时改变头数以保持每头32维；短上下文组每步训练字符数下降。报告的质量星级来自对全部保存文本的主观评阅，标准与评分见 `report/quality_ratings.json`，不代表原创性或人类盲评。

2026-09-23提交前复查：6项测试通过，47项必要产物哈希匹配，9个模型及1000步快照CPU重放通过，58段原始文本与统计一致。HPC队列为空，10765及子步骤均COMPLETED、退出码0:0；本轮未重新训练。
